...
generate_plot_signature <- function(plot_obj) {
  safe_digest <- function(obj) {
    tryCatch(
      digest::digest(obj, algo = "md5"),
      error = function(e) digest::digest(capture.output(utils::str(obj)), algo = "md5")
    )
  }
  if (inherits(plot_obj, "ggplot")) {
    ...
    return(safe_digest(paste(sig_parts, collapse = "||")))
    ...
    return(paste0("recordedplot_", safe_digest(plot_obj))) # Hash the object itself
  } else if (is.list(plot_obj) && "call" %in% names(plot_obj)) {
    # For base R plots stored as lists with a call
    return(safe_digest(deparse(plot_obj$call)))
  }
  return(safe_digest(plot_obj)) # Fallback: hash the object
}
